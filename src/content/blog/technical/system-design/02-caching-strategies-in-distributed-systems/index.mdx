...
<Mermaid code={`flowchart LR
  ...
  App[App] --> Cache[Cache]
  Cache --> keys
  keys -->|"all expire together"| DB[(Database)]
`} />
...
<Mermaid code={`flowchart LR
  subgraph before["Without jitter: all expire at T"]
    B1[key1] --> T1[T]
    B2[key2] --> T1
    B3[keyn] --> T1
  end
  subgraph after["With jitter: expiry spread"]
    A1[key1] --> T2[T-30s]
    A2[key2] --> T3[T]
    A3[keyn] --> T4[T+30s]
  end
...
<Mermaid code={`flowchart LR
  ...
  Cache -->|"return immediately"| User[User]
  Cache -.->|"with probability p"| Backend[Background refresh]
  Backend -.-> Cache
  subgraph window["Early refresh window"]
    W[last 1-2 min of TTL]
  end
`} />
...
<Mermaid code={`flowchart LR
  ...
  Cache -->|"return stale immediately"| User[User]
  Cache -.->|"async revalidate"| Backend[Backend]
  Backend -.-> Cache
  Next[Next request] --> Cache
  Cache -->|"fresh (or stale)"| NextUser[User]
`} />
...
<Mermaid code={`flowchart LR
  subgraph phase1["Post-deploy / Before traffic"]
    Job[Warming job] --> Cache[Cache]
    Job -->|"pre-fill hot keys"| Cache
  end
  subgraph phase2["Traffic arrives"]
    Traffic[Users] --> Cache
    Cache -->|"cache hit"| Traffic
  end
  phase1 --> phase2
`} />
...
<Mermaid code={`flowchart TB
  subgraph tradeoffs["Pick your tradeoffs"]
    F[Freshness]
    L[Latency]
    C[Consistency]
  end
  ...
  C --- F
  F --> FL[more refills\nmore backend load]
  L --> LL[serve from cache\naccept staleness]
  C --> CL[strong consistency\n= expensive]
`} />
...
<Mermaid code={`flowchart TD
  Start([Start]) --> Q1{High traffic\nsame hot key?}
  Q1 -->|Yes| Jitter[Jitter + Mutex]
  Q1 -->|No| Q2{Read-heavy\nuser-facing?}
  Q2 -->|Yes| SWR[SWR]
  Q2 -->|No| Q3{Cold start\nor after deploy?}
  Q3 -->|Yes| Warm[Cache warming]
  Q3 -->|No| Q4{Smooth load\nbefore expiry?}
  Q4 -->|Yes| Early[Prob. early expiration]
`} />
